...
public class UserControllerTest extends BaseSpringJUnit4Test{
	...
	public void testGetList() throws Exception {
	    mockMvc.perform(get("/users?page=0&size=10&sort=id,asc&sort=username,desc&username=user")
	            .accept(MediaTypes.HAL_JSON)) //执行请求
	            .andExpect(status().isOk()) //200
	            .andExpect(content().contentType(MediaTypes.HAL_JSON)); //验证响应contentType
	    mockMvc.perform(get("/users?page=0&size=10&sort=id,asc&sort=username,desc&email=10")
	            .accept(MediaTypes.HAL_JSON)) //执行请求
	            .andExpect(status().isOk()) //200
	            .andExpect(content().contentType(MediaTypes.HAL_JSON)); //验证响应contentType
	...
	public void testGet() throws Exception {
		mockMvc.perform(get("/users/{id}",1L).accept(MediaTypes.HAL_JSON))
	            .andExpect(status().isOk()) //200
	            .andExpect(content().contentType(MediaTypes.HAL_JSON)) //验证响应contentType
...
//	    requestBody = "{\"id\":"+id+",\"username\":\"user" + id + "\",\"password\":\"123456\",\"roles\":[1]}";
	...
	public void test3Delete() throws Exception {
	    ...
	    mockMvc.perform(delete("/users/?id={id}", 0L)
	            .accept(MediaType.APPLICATION_JSON)) //执行请求
	            .andExpect(status().isInternalServerError()) //500错误请求
	            .andExpect(jsonPath("$.code").value(500 + "")) //使用Json path验证JSON
	            .andReturn();
